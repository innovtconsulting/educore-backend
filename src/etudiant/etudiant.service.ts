...
import { applyNormalizedSearch } from '../common/utils/search.util';
...
export class EtudiantService {
  ...
  async findAll(
    ...
    const query = this.etudiantRepository
      ...
      .leftJoinAndSelect('etudiant.parents', 'parents')
      .leftJoinAndSelect('etudiant.user', 'user');

    if (status) query.andWhere('etudiant.status = :status', { status });
    if (tenantId) {
      query.andWhere('etablissement.id = :tenantId', { tenantId });
    } else if (etablissementId) {
      query.andWhere('etablissement.id = :etablissementId', {
        etablissementId,
      });
    }
    ...
    applyNormalizedSearch(
      query,
      search,
      [
        'etudiant."lastName"',
        'etudiant."firstName"',
        'concat_ws(\' \', etudiant."lastName", etudiant."firstName")',
        'concat_ws(\' \', etudiant."firstName", etudiant."lastName")',
        'etudiant.matricule',
        'etudiant.email',
      ],
      'studentSearch',
