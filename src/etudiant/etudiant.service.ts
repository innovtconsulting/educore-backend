...
export class EtudiantService {
  ...
  async update(
    ...
    if (parentsData && parentsData.length > 0) {
      ...
      for (const pData of parentsData) {
        ...
        if (!parent) {
          const parentEtablissementId =
            etablissementId || etudiant.etablissement?.id;
          parent = this.parentRepository.create({
            ...pData,
            etablissementId: parentEtablissementId,
          });
          parent = await this.parentRepository.save(parent);
        }
        parents.push(parent);
  ...
  private normalizeGender(raw: string | undefined): 'M' | 'F' | undefined {
    ...
    const v = raw.trim().toLowerCase().normalize('NFD').replace(/[̀-ͯ]/g, ''); // strip accents
    const masculin = [
      'm',
      'h',
      'male',
      ...
      for (const worksheet of workbook.worksheets) {
        ...
        for (let i = 2; i <= worksheet.rowCount; i++) {
          ...
          try {
            ...
            const savedEtudiantRun = await queryRunner.manager.save(etudiant);
            if (email) {
              await this.userService.createWithRunner(queryRunner, {
                email,
                password: '12345678',
