...
describe('ParentDashboard (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    await dataSource.query('TRUNCATE "user", "parent", "etudiant", "classe", "niveau", "etablissement", "matiere", "emploi_du_temp", "presence", "facture", "paiement" CASCADE');
    ...
    await dataSource.query(`INSERT INTO classe_etablissements_etablissement ("classeId", "etablissementId") VALUES (${classe[0].id}, ${etablissementId})`);
    await dataSource.query(`INSERT INTO classe_niveaux_niveau ("classeId", "niveauId") VALUES (${classe[0].id}, ${niveau[0].id})`);

    const etudiant = await dataSource.query(`INSERT INTO etudiant ("firstName", "lastName", "email", "matricule", "etablissementId", "classeId", "niveauId") VALUES ('Enfant', 'Test', 'enfant@test.com', 'MAT123', ${etablissementId}, ${classe[0].id}, ${niveau[0].id}) RETURNING id`);
    ...
    await dataSource.query(`INSERT INTO etudiant_parents_parent ("etudiantId", "parentId") VALUES (${etudiantId}, ${parentId})`);

    // Créer une matière et un cours pour aujourd'hui
    const matiere = await dataSource.query(`INSERT INTO matiere (name, code, coefficient) VALUES ('Algorithmique', 'ALG101', 3) RETURNING id`);
    const today = new Date();
    today.setHours(8, 0, 0, 0);
    const end = new Date(today);
    end.setHours(10, 0, 0, 0);

    const emploi = await dataSource.query(`INSERT INTO emploi_du_temp ("startTime", "endTime", "matiereId", "etablissementId", "classeId", "niveauId") VALUES ('${today.toISOString()}', '${end.toISOString()}', ${matiere[0].id}, ${etablissementId}, ${classe[0].id}, ${niveau[0].id}) RETURNING id`);

    // Créer une absence pour aujourd'hui
    await dataSource.query(`INSERT INTO presence (status, "etudiantId", "emploiDuTempId") VALUES ('Absent', ${etudiantId}, ${emploi[0].id})`);

    // Créer une facture impayée
    await dataSource.query(`INSERT INTO facture (numero, "montantTotal", status, "dateEmission", "etudiantId") VALUES ('FAC-001', 500000, 'Validée', NOW(), ${etudiantId})`);
