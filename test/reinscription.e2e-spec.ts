...
describe('Réinscription (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    const anneePassée = await anneeRepo.save({
      label: '2025-2026',
      startDate: '2025-10-01',
      endDate: '2026-06-30',
      isActive: false,
    });
    const anneeNouvelle = await anneeRepo.save({
      label: '2026-2027',
    ...
    const loginRes = await request(app.getHttpServer())
      .post('/api/auth/login')
      .send({ email: 'admin@espm.sn', password: 'password123' });
  ...
  it('should allow reinscription if all criteria met (academic & financial)', async () => {
    ...
    const newInvoices = await facRepo.findBy({
  ...
  it('should mark a student as graduated if all criteria met', async () => {
    ...
    const updatedIns = await insRepo.findOneBy({
      etudiant: { id: etudiant.id },
      anneeUniversitaire: { id: 1 },
    });
