...
describe('Note Bulk Entry (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    await dataSource.query(
      'TRUNCATE "user", "enseignant", "affectation", "etudiant", "matiere", "niveau", "classe", "etablissement", "evaluation", "note", "semestre", "annee_universitaire" CASCADE',
    );

    // 1. Setup environnement
    const etablissement = await dataSource.query(
      `INSERT INTO etablissement (name, address, email) VALUES ('ESPM Bulk', 'Dakar', 'bulk@espm.sn') RETURNING id`,
    );
    const etablissementId = etablissement[0].id;

    const niveau = await dataSource.query(
      `INSERT INTO niveau (name) VALUES ('L1 Bulk') RETURNING id`,
    );
    const niveauId = niveau[0].id;

    const classe = await dataSource.query(
      `INSERT INTO classe (name) VALUES ('Informatique Bulk') RETURNING id`,
    );
    ...
    const matiere = await dataSource.query(
      `INSERT INTO matiere (name, code, coefficient) VALUES ('Algorithmique Bulk', 'ALG_BULK', 3) RETURNING id`,
    );
    ...
    const enseignant = await dataSource.query(
      `INSERT INTO enseignant ("firstName", "lastName", "email", "matricule", "dateEmbauche") VALUES ('Prof', 'Bulk', 'prof.bulk@test.com', 'ENS_BULK', NOW()) RETURNING id`,
    );
    ...
    const etudiant1 = await dataSource.query(
      `INSERT INTO etudiant ("firstName", "lastName", "email", "matricule", "etablissementId", "classeId", "niveauId") VALUES ('Etudiant', '1', 'et1@test.com', 'MAT_B1', ${etablissementId}, ${classeId}, ${niveauId}) RETURNING id`,
    );
    etudiant1Id = etudiant1[0].id;
    const etudiant2 = await dataSource.query(
      `INSERT INTO etudiant ("firstName", "lastName", "email", "matricule", "etablissementId", "classeId", "niveauId") VALUES ('Etudiant', '2', 'et2@test.com', 'MAT_B2', ${etablissementId}, ${classeId}, ${niveauId}) RETURNING id`,
    );
    etudiant2Id = etudiant2[0].id;

    const annee = await dataSource.query(
      `INSERT INTO annee_universitaire (label, "startDate", "endDate") VALUES ('2025-2026', '2025-10-01', '2026-07-31') RETURNING id`,
    );
    const semestre = await dataSource.query(
      `INSERT INTO semestre (name, "startDate", "endDate", "anneeUniversitaireId") VALUES ('S1 Bulk', '2025-10-01', '2026-02-28', ${annee[0].id}) RETURNING id`,
    );

    const evaluation = await dataSource.query(
      `INSERT INTO evaluation (title, type, date, "matiereId", "classeId", "niveauId", "semestreId") VALUES ('CC1 Bulk', 'Contrôle Continu', NOW(), ${matiereId}, ${classeId}, ${niveauId}, ${semestre[0].id}) RETURNING id`,
    );
    ...
    await dataSource.query(
      `INSERT INTO "user" (email, password, role, "enseignantId") VALUES ('prof.bulk@test.com', '${hashedPassword}', 'Enseignant', ${enseignantId})`,
    );
  ...
  it('/note/bulk (POST) - Enseignant saisi les notes en masse pour sa matière', async () => {
    ...
    const notes = await dataSource.query(
      `SELECT * FROM note WHERE "evaluationId" = ${evaluationId}`,
    );
  ...
  it('/note/bulk (POST) - Ne doit pas permettre de saisir pour une évaluation hors affectation', async () => {
    ...
    const matiere2 = await dataSource.query(
      `INSERT INTO matiere (name, code, coefficient) VALUES ('Matière Interdite', 'FORBIDDEN', 2) RETURNING id`,
    );
    const annee = await dataSource.query(
      `SELECT id FROM annee_universitaire LIMIT 1`,
    );
    const semestre = await dataSource.query(`SELECT id FROM semestre LIMIT 1`);
    const etablissement = await dataSource.query(
      `SELECT id FROM etablissement LIMIT 1`,
    );
    ...
    const evalForbidden = await dataSource.query(
      `INSERT INTO evaluation (title, type, date, "matiereId", "classeId", "niveauId", "semestreId") VALUES ('CC Interdit', 'Contrôle Continu', NOW(), ${matiere2[0].id}, ${classe[0].id}, ${niveau[0].id}, ${semestre[0].id}) RETURNING id`,
    );
