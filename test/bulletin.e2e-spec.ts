...
import {
  EvaluationType,
  EvaluationSession,
} from './../src/evaluation/entities/evaluation.entity';
...
describe('Bulletin Module (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    await dataSource.query(
      'TRUNCATE "user", "parent", "etudiant", "classe", "niveau", "etablissement", "matiere", "evaluation", "note", "annee_universitaire", "semestre" RESTART IDENTITY CASCADE;',
    );
    ...
    const etablissement = await dataSource.query(
      `INSERT INTO etablissement (name, address, email) VALUES ('Etab Bulletin', 'Test', 'bull@test.com') RETURNING id`,
    );
    etablissementId = etablissement[0].id;

    const niveau = await dataSource.query(
      `INSERT INTO niveau (name) VALUES ('L1 Bulletin') RETURNING id`,
    );
    niveauId = niveau[0].id;

    const classe = await dataSource.query(
      `INSERT INTO classe (name) VALUES ('Classe Bulletin') RETURNING id`,
    );
    ...
    const matiere = await dataSource.query(
      `INSERT INTO matiere (name, code, coefficient) VALUES ('Maths Bulletin', 'MATH101', 5) RETURNING id`,
    );
    ...
    const parent = await dataSource.query(
      `INSERT INTO parent ("firstName", "lastName", "email", "phoneNumber", "gender") VALUES ('P', 'P', 'parent@test.com', '000', 'Père') RETURNING id`,
    );

    const etudiant = await dataSource.query(
      `INSERT INTO etudiant ("firstName", "lastName", "email", "matricule", "etablissementId", "classeId", "niveauId") VALUES ('Jean', 'Note', 'jean.note@test.com', 'JEAN-001', ${etablissementId}, ${classeId}, ${niveauId}) RETURNING id`,
    );
    etudiantId = etudiant[0].id;
    await dataSource.query(
      `INSERT INTO etudiant_parents_parent ("etudiantId", "parentId") VALUES (${etudiantId}, ${parent[0].id})`,
    );

    const annee = await dataSource.query(
      `INSERT INTO annee_universitaire (label, "startDate", "endDate") VALUES ('2026-2027', '2026-10-01', '2027-07-31') RETURNING id`,
    );
    const anneeId = annee[0].id;

    const semestre = await dataSource.query(
      `INSERT INTO semestre (name, "startDate", "endDate", "anneeUniversitaireId") VALUES ('S1 Test', '2026-10-01', '2027-02-28', ${anneeId}) RETURNING id`,
    );
